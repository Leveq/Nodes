...
| **IP address exposure** | Partially mitigated | Your IP is always visible to relay peers (Nodes does not route through anonymizing networks). For voice channels, the client defaults to routing all WebRTC traffic through the LiveKit SFU so that other participants do not see your IP. Users can opt in to P2P mesh voice in Settings → Voice → Voice Privacy for lower latency, which re-exposes IPs to other participants. Use a VPN or Tor if relay-level IP privacy is also required. |
