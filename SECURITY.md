...
| **IP address exposure** | Partially mitigated | Your IP is always visible to relay peers (Nodes does not route through anonymizing networks). For voice channels, the client prefers routing WebRTC traffic through the LiveKit SFU so that other participants do not see your IP; if no SFU is configured for the Node or the SFU join fails, it may fall back to P2P mesh (re-exposing IPs). Users can also opt in to P2P mesh voice in Settings → Voice → Voice Privacy for lower latency, which re-exposes IPs to other participants. Use a VPN or Tor if relay-level IP privacy is also required. |
