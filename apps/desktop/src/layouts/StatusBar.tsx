 ...
 */
export function StatusBar({ onOpenSettings, onOpenProfile, updateAvailable }: StatusBarProps) {
      ...
      <div className="flex items-center gap-3">
        ...
        {onOpenSettings && (
          <button
            ...
            className="relative p-1 text-nodes-text-muted hover:text-nodes-text transition-colors"
            title={updateAvailable ? "Settings — update available" : "Settings (Ctrl+,)"}
          >
            ...
            {updateAvailable && (
              <span className="absolute top-0 right-0 w-2 h-2 bg-nodes-danger rounded-full" />
            )}
          </button>
        )}
      </div>
